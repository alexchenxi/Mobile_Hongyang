                    ...
                    <van-tab title="我的考勤" >
                        <div class="records-container">
                            ...
                            <div class="records-details">
                                <van-empty v-if="attendanceList.length === 0 && isFinished" class="custom-image" :image="$baseImgUrl + '/no-data.png'" description="当天没有签到记录~" />
								<section v-else>
                                    <template v-if="$route.query.type === 'staff'">
                                    ...
                                    <template v-else>
                                        <div class="list">
                                            <div class="list-item" v-for="item in attendanceList" :key="item.userId" @click="goDetail(item)">
                                                <div class="avatar">
                                                ...
                                                <div class="desc">
                                                    ...
                                                    <div class="desc-time" v-if="item.detail && item.detail.length">
                                                        <div class="time-item" >
                                                            ...
                                                            <span class="btn-status">{{ item.detail[0].type ? '任务' : '考勤' }}</span>
                                                        </div>
                                                        <div class="time-item"  v-if="item.detail.length >= 2">
                                                            <span class="time-col">下班时间：</span>
                                                            {{ item.detail[1].signAt }}
                                                            <span class="btn-status">{{ item.detail[1].type ? '任务' : '考勤' }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
...
export default {
    name: 'selfChannelOrCounselorsignAgent',
    components: {
        TopBar,
        AttendanceCalendar
    },
    ...
    created() {
        if (this.$route.query.type == 'selfChanne') {
            this.title = '自渠出勤人次'
        }else {
            this.title = '置业顾问出勤人次'
        }
    ...
    methods: {
        ...
        async getAttendanceRecords() {
            ...
            let result
            if (this.$route.query.type === 'selfChanne') {
                result = await API.getSelfChannelSignStaffList(params)
            }else {
                result = await API.getReportSignStaffList(params)
            }
