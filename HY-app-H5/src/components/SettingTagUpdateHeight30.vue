<template>
  <div class="setting-tag-update">
    <div
      class="setting-tag-button"
      @click.stop="depictBtn"
    >
      <img
        class="icon-tag"
        :src="$baseImgUrl + '/case/customer/icon-tag.png'"
        alt=""
      >
    ...
    <van-dialog
      v-model="depictShow"
      :title="'设置客户描摹'"
      class="customer-depict-content"
      show-cancel-button
      :before-close="handleDepictClose"
      get-container="body"
    >
      <div
        v-show="depictShowTip"
        class="tip"
      >
        <span class="iconfont icontishi2" /> 最多可以维护15个标签，已经维护{{ depictClientNumber }}个啦~
      </div>
      <p class="customer-depict-content-p">
        已维护标签
      </p>
      <div class="customer-depict-iconbox">
        <van-button
          ...
          :key="item.id"
          :class="{'active':depictId== item.id}"
          class="customer-depict-icon"
          :title="item.tagName"
          plain
          hairline
          type="info"
          @click.stop="depictClickTag(item.id)"
        >
          {{ item.tagName }}
        </van-button>
        <van-button
          ...
          plain
          icon="plus"
          class="customer-depictadd-btn customer-depict-icon"
          type="info"
          @click.stop="depictAddTag()"
        >
          添加标签
        </van-button>
      </div>
