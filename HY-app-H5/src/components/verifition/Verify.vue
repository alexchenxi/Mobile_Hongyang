<template>
  <div v-show="showBox" :class="mode=='pop'?'mask':''">
    <div :class="mode=='pop'?'verifybox':''" :style="{'max-width':parseInt(imgSize.width)+36+'px'}">
      <div v-if="mode=='pop'" class="verifybox-top">
        滑动验证
        <span class="verifybox-close" @click="closeBox">
          <i class="iconfont icondel" />
        </span>
      </div>
      <div class="verifybox-bottom" :style="{padding:mode=='pop'?'18px 18px 26px 18px':'0'}">
        <!-- 验证码容器 -->
        <components
          :is="componentType"
          v-if="componentType"
...
export default {
  ...
  created() {
    this.defaultImg = this.$baseImgUrl + '/default.png'
  },
  mounted() {
    this.uuid()
  },
...
<style lang="less">
    ...
    .verify-bar-area .verify-move-block {
        ...
        background-image: url("@{baseImgUrl}/slide-0.png");
        background-size: 100% 100%;
        border-radius: 50% 50%;
        &.draging {
          background-image: url("@{baseImgUrl}/slide-1.png");
          background-size: 100% 100%;
        }
        &.dragEnd {
          background-image: url("@{baseImgUrl}/slide-2.png");
          background-size: 100% 100%;
        }
    }
    ...
    .verify-img-panel .iconshuaxin {
        font-size: 18px;
        color: #fff;
    }
    .verify-img-panel .black {
        color: #999;
    }
